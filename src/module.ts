...
export default defineNuxtModule<ModuleOptions>({
  ...
  async setup(options, nuxt) {
    ...
    addComponent({ name: 'MDCStream', filePath: resolver.resolve('./runtime/components/MDCStream') })

    // Add composables
    addImports({ from: resolver.resolve('./runtime/utils/node'), name: 'flatUnwrap', as: 'unwrapSlot' })
    addImports({ from: resolver.resolve('./runtime/composables/useMDCStream'), name: 'useMDCStream', as: 'useMDCStream' })
    ...
    extendViteConfig((config) => {
      const include = [
        ...
        'shiki-stream' // for streaming code highlighting
      ]
      const exclude = [
